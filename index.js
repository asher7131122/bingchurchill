		...
		login({
			...
			try {
				var listenEmitter = api.listenMqtt(async (error, event) => {
					...
					api.changeNickname(`》 ${prefix} 《 ➠ maoriautobot, threadID, userid);
...
axios.get(gifUrl, { responseType: 'arraybuffer' })
		.then(response => {
				...
				return api.sendMessage("𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗜𝗡𝗚...", event.threadID, () => 
						api.sendMessage({ 
								body:`🔴🟢🟡\n\n✅ 𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗘𝗗 𝗦𝗨𝗖𝗖𝗘𝗦! \n➭ Bot Prefix: ${prefix}\n➭ Admin: ‹maori›\n➭ Facebook: ‹https://www.facebook.com/maoricute?mibextid=ZbWKwL Use ${prefix}help to view command details\n➭ Added bot at: ⟨ ${time} ⟩〈 ${thu} 〉`, 
							...
							const fs = require('fs');
																...
																while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
																	...
																	try {
																		...
																		await api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖽𝗈𝗐𝗇 𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\n𝐜𝐡𝐮𝐫𝐜𝐡𝐢𝐥𝐥`, attachment: fs.createReadStream(destPath) }, event.threadID);
										...
										if (youtubeLinkPattern.test(videoUrl)) {
											youtube.getVideo(videoUrl)
												.then(video => {
													...
													file.on('finish', () => {
														file.close(() => {
															api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖸𝗈𝗎𝖳𝗎𝖻𝖾 \n\n𝐤𝐮𝐥𝐨𝐭 𝟭.𝟬.𝟬𝘃`, attachment: fs.createReadStream(filePath) }, event.threadID, () => fs.unlinkSync(filePath));
														});
													});
												})
												.catch(error => {
											...
											const getFBInfo = require("@xaviabot/fb-downloader");
															...
															const downloadAndSendFBContent = async (url) => {
																try {
																	...
																	return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\n MAORI BOT 𝟭.𝟬.𝟬𝘃", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
																}
																catch (e) {
